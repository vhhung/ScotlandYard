...
final class LocalSetupController implements Controller {
	...
	@FXML private Slider mrXTimeout;
	@FXML private Label mrXTimeoutHint;
	@FXML private Slider detectiveTimeout;
	@FXML private Label detectiveTimeoutHint;
...
//		current.aiProperty().bind(playerAI.getSelectionModel().selectedItemProperty());
	...
	private void bindRoundConfig(ModelProperty initialValue) {
		// timeout
		mrXTimeoutHint.textProperty().bind(EasyBind.map(mrXTimeout.valueProperty(), Number::doubleValue)
				.map(Math::round).map(String::valueOf));
		mrXTimeout.valueProperty().setValue(initialValue.mrXTimeoutProperty().get().getSeconds());

		detectiveTimeoutHint.textProperty().bind(EasyBind.map(detectiveTimeout.valueProperty(), Number::doubleValue)
				.map(Math::round).map(String::valueOf));
		detectiveTimeout.valueProperty().setValue(initialValue.detectiveTimeoutProperty().get().getSeconds());
	...
	ModelProperty createGameConfig() {
		...
		return new ModelProperty(
				Duration.ofSeconds(Math.round(mrXTimeout.getValue())),
				Duration.ofSeconds(Math.round(detectiveTimeout.getValue())),
